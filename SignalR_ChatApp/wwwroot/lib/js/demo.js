"use strict"
var hubConnection = new signalR.HubConnectionBuilder().withUrl("/videoCallingHub").build();
var Demo = (function () {
    var _audioTrack;
    ...
    hubConnection.on("ReceiveMessage", async function (messagerec) {
        console.log('message', messagerec);
        var message = JSON.parse(messagerec);
        ...
        if (message.rejected) {
            ...
            if (!_audioTrack) {
                ...
                if (r) {
                    ...
                    hubConnection.invoke("SendMessage", JSON.stringify({ 'rejected': 'true' }));
                }
            }
            if (_audioTrack) {
                ...
                hubConnection.invoke("SendMessage", JSON.stringify({ 'answer': connection.setLocalDescription }));
    ...
    async function _createConnection() {
        ...
        connection.onicecandidate = function (event) {
            ...
            if (event.candidate) {
                ...
                hubConnection.invoke("SendMessage", JSON.stringify({ 'iceCandidate': event.candidate }));
            }
        }
    ...
    async function _createOffer() {
        ...
        hubConnection.invoke("SendMessage", JSON.stringify({ 'offer': connection.localDescription }));
    }
